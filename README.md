...
Throughout your report make sure you always include the code that you used to generate the output you present. When writing code chunks in the R markdown document, always use ???????? = ???????? so that someone else will be able to read the code. *This assignment will be evaluated via peer assessment so it is essential that your peer evaluators be able to review the code for your analysis.**
...
1. Commit your completed ??????_????????????????.?????? file to the ???????????? branch of your git repository (you should already be on the  ???????????? branch unless you created new ones)
